...
export function buildMomHtml(data) {
  ...
  const runningHeaderTitle = `${escapeHtml(epName)}'s Check-In Call MINUTES OF MEETING | `;
...
<style>
  * {
  ...
  /* ---------- LAYOUT PAGE FOR SCREEN PREVIEW ---------- */
  .page {
    width: 210mm;
    min-height: 297mm;
    position: relative;
    box-sizing: border-box;
    padding: 1.1in 1in 1in 1in;
    background: #ffffff;
  ...
  /* ---------- ATURAN HALAMAN PRINT & RUNNING HEADER ---------- */
  @page {
    size: A4 portrait;
    margin-top: 1.1in; /* Ruang atas untuk penempatan header */
    margin-bottom: 0.8in;
    margin-left: 1in;
    margin-right: 1in;
    
    @top-right {
      content: "${runningHeaderTitle}" "AIESEC in Universitas Indonesia";
      font-family: 'Lato', Arial, sans-serif;
      font-size: 10.5pt;
      font-style: italic;
      color: #00b0f0;
      padding-bottom: 0.25in; /* Jarak antara header ke konten */
    }
    @bottom-right {
  ...
  /* Hilangkan Header & Footer di Halaman 1 (Cover) */
  @page :first {
    margin: 0;
  ...
  /* SUPAYA TABEL TIDAK KEPOTONG / NABRAK ATAS SAAT PINDAH HALAMAN */
  table {
    width: 100%;
  ...
  /* ---------- HEADER DIPAKAI KHUSUS MODE SCREEN/PREVIEW ---------- */
  .screen-header {
    display: block;
    position: absolute;
    top: 0.45in;
  ...
  @media print {
    .screen-header {
      display: none; /* Menggunakan native @page header saat di-print/PDF */
    }
  }
...
<body>
  ...
  <div class="page">
    <div class="screen-header">
      ${escapeHtml(epName)}'s Check-In Call MINUTES OF MEETING | <span class="brand">AIESEC in Universitas Indonesia</span>
    </div>
