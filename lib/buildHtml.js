...
export function buildMomHtml(data) {
  ...
  const epHeader = `${escapeHtml(epName)}'s Check-In Call MINUTES OF MEETING `;
  const brandHeader = `| AIESEC in Universitas Indonesia`;
...
<style>
  * {
  ...
  /* ---------- MARGIN & HEADER UNTUK SETIAP HALAMAN CETAK (2, 3, DST) ---------- */
  @page {
    size: A4 portrait;
    margin-top: 0.8in;
    margin-bottom: 0.8in;
    @top-right {
      content: "${epHeader} ${brandHeader}";
      font-family: 'Lato', Arial, sans-serif;
      font-size: 10.5pt;
      font-style: italic;
      color: #00b0f0;
      padding-right: 0.98in;
      padding-top: 0.5in;
    }
    @bottom-right {
  ...
  /* Matikan Header & Nomor Halaman di Halaman 1 (Cover) */
  @page :first {
    margin: 0;
  ...
  @media print {
    ...
    /* Sembunyikan header manual HTML di mode cetak karena sudah dicover oleh @top-right */
    .doc-header {
      display: none;
    }
  }

  /* ATURAN SUPAYA TABEL OTOMATIS PINDAH HALAMAN SAAT MENTOK */
  table {
    width: 100%;
...
<body>
  ...
  <!-- ===================== PAGE 2+: MAIN CONTENT ===================== -->
  <div class="page">
    <!-- Header visual untuk preview tampilan web/screen -->
    <div class="doc-header">${epHeader} <span class="brand">${brandHeader}</span></div>
